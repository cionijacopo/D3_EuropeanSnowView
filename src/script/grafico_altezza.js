...
const chartContainer = d3.select("#cella-1-3")
    ...
    .attr("id", "altitudeContainer")
     // Inizialmente nascosto, stile flessibile centrato
    .style("display", "none")
    .style("width", "100%")
    .style("height", "100%")
...
// Margini e dimensioni del grafico (area interna al SVG)
...
// Tooltip per mostrare dettagli al passaggio su una barra
...
// Funzione che aggiorna il bar chart in base al valore massimo di altitudine selezionato
function updateAltitudeChart(data, maxAltitude) {
    svg.selectAll("*").remove(); // pulizia grafico precedente 

    // definizione bin 
    const bins = [0, 1000, 1250, 1500, 1750, 2000, 2500, 3000];
    // Filtra i resort in base al valore massimo selezionato e alla validità dei dati
    const filtered = data.filter(d => +d.HighestPoint && +d.TotalSlope && +d.HighestPoint <= maxAltitude);

    // Raggruppa i dati per intervallo di altitudine e calcola somma delle piste, numero resort, nomi
...
// Funzione che disegna e mostra il grafico altitudini per uno stato selezionato
function drawAltitudeChart(stateCode) {
    const file = `data/resorts_by_country/coordinates/${stateCode}_with_coordinates.csv`;
    d3.csv(file).then(data => {
        // Nasconde lo spiderplot e mostra il contenitore del grafico altitudini
        ...
        // Disegna grafico e aggiorna anche quello delle piste
...
// Nasconde il grafico e ripristina lo spiderplot
